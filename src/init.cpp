...
//    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BWK and BWKz money supply statistics") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, bulwark, (obfuscation, swifttx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Bulwark specific functionality (Masternodes, Zerocoin, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupbwkz=<n>", strprintf(_("Enable automatic wallet backups triggered after each BWKz minting (0-1, default: %u)"), 1));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (!filesystem::exists(backupDir)) {
            // Always create backup directory to not confuse the operating system's file browser
        ...
        if (GetBoolArg("-resync", false)) {
            ...
            // Delete the local blockchain directories to force a resync from scratch to get a consistent blockchain-state
            ...
            LogPrintf("Deleting blockchain directories: blocks, chainstate, sporks and zerocoin\n");
            // We delete in 4 individual steps in case one of the directories is missing already
            try {
                if (filesystem::exists(blocksDir)){
                    boost::filesystem::remove_all(blocksDir);
                    LogPrintf("-resync: directory deleted: %s\n", blocksDir.string().c_str());
                }
                ...
                LogPrintf("Failed to delete blockchain directories %s\n", error.what());
            }
        }
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Populate list of invalid/fraudulent outpoints that are banned from the chain (Bulwark: Not Applicable)
