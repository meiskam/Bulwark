                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->BWKZpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelBWKZSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_18_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintBWKZ->setEnabled(false);
        ui->pushButtonMintBWKZ->setToolTip(tr("BWKz is currently disabled due to maintenance."));

        ui->pushButtonSpendBWKZ->setEnabled(false);
        ui->pushButtonSpendBWKZ->setToolTip(tr("BWKz is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->BWKZpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintBWKZ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_18_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("BWKz is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " BWKz...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" BWKz in ") +
...
void PrivacyDialog::on_pushButtonSpendBWKZ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_18_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("BWKz is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, send BWKz
        sendBWKZ();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send BWKz
    sendBWKZ();
}

void PrivacyDialog::on_pushButtonBWKZControl_clicked()
{
    BWKZControlDialog* BWKZControl = new BWKZControlDialog(this);
    BWKZControl->setModel(walletModel);
    BWKZControl->exec();
}

void PrivacyDialog::setBWKZControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelBWKZSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendBWKZ()
{
    ...
    double dAmount = ui->BWKZpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as BWKZ is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " BWKz</b>";
    ...
    // use mints from BWKz selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!BWKZControlDialog::listSelectedMints.empty()) {
        vMintsSelected = BWKZControlDialog::GetSelectedMints();
    }
    ...
    // Clear bwkz selector in case it was used
    BWKZControlDialog::listSelectedMints.clear();
    ui->labelBWKZSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("BWKz Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " BWKz </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" BWKz "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" BWKz "));
    ui->labelBWKZAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>BWKz </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " BWKz </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelBWKZSyncStatus->setVisible(fShow);
}
