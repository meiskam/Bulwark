...
#include "ui_bwkzcontroldialog.h"
...
std::list<std::string> BWKZControlDialog::listSelectedMints;
std::list<CZerocoinMint> BWKZControlDialog::listMints;

BWKZControlDialog::BWKZControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::BWKZControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
BWKZControlDialog::~BWKZControlDialog()
{
    delete ui;
}

void BWKZControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void BWKZControlDialog::updateList()
{
...
void BWKZControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void BWKZControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelBWKZ_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setBWKZControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> BWKZControlDialog::GetSelectedMints()
{
...
void BWKZControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
