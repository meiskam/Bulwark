...
bool CMasternodeDB::Write(const CMasternodeMan& mnodemanToSave)
{
    ...
    LogPrintf("masternode", "Written info to mncache.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodeDB::ReadResult CMasternodeDB::Read(CMasternodeMan& mnodemanToLoad, bool fDryRun)
{
    ...
    LogPrintf("masternode", "Loaded info from mncache.dat  %dms\n", GetTimeMillis() - nStart);
    LogPrintf("masternode", "  %s\n", mnodemanToLoad.ToString());
    if (!fDryRun) {
        LogPrintf("masternode", "Masternode manager - cleaning....\n");
        mnodemanToLoad.CheckAndRemove(true);
        LogPrintf("masternode", "Masternode manager - result:\n");
        LogPrintf("masternode", "  %s\n", mnodemanToLoad.ToString());
    }
...
void DumpMasternodes()
{
    ...
    LogPrintf("masternode", "Verifying mncache.dat format...\n");
    CMasternodeDB::ReadResult readResult = mndb.Read(tempMnodeman, true);
    // there was an error and it was not an error on file opening => do not proceed
    if (readResult == CMasternodeDB::FileError)
        LogPrintf("masternode", "Missing masternode cache file - mncache.dat, will try to recreate\n");
    else if (readResult != CMasternodeDB::Ok) {
        LogPrintf("masternode", "Error reading mncache.dat: ");
    ...
    LogPrintf("masternode", "Writting info to mncache.dat...\n");
    mndb.Write(mnodeman);

    LogPrintf("masternode", "Masternode dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        if (pnode->fObfuScationMaster) {
	    	...
	    	LogPrintf("masternode", "Closing Masternode connection peer=%i \n", pnode->GetId());
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (!obfuScationSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyCollateralAddress)) {
            LogPrintf("masternode", "mnb - Got mismatched pubkey and vin\n");
	...
	LogPrint("masternode", "mnp - Masternode ping, vin: %s\n", mnp.vin.prevout.hash.ToString());
            ...
            if (!isLocal && Params().NetworkID() == CBaseChainParams::MAIN) {
                ...
                if (i != mAskedUsForMasternodeList.end()) {
                    ...
                    if (GetTime() < t) {
                        ...
                        LogPrintf("masternode", "dseg - peer already asked me for the list\n");
                        return;
                    }
                }
                int64_t askAgain = GetTime() + MASTERNODES_DSEG_SECONDS;
        ...
        if (vin == CTxIn()) {
            ...
            LogPrintf("masternode", "dseg - Sent %d Masternode entries to %s\n", nInvCount, pfrom->addr.ToString());
        }
    }
    /*
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
	    ...
	    LogPrintf("masternode", "dsee - Signature rejected, too far into the future %s\n", vin.prevout.hash.ToString());
        ...
        if (pubkeyScript.size() != 25) {
            LogPrintf("masternode", "dsee - pubkey the wrong size\n");
        ...
        if (pubkeyScript2.size() != 25) {
            LogPrintf("masternode", "dsee - pubkey2 the wrong size\n");
            Misbehaving(pfrom->GetId(), 100);
            return;
        }

        if (!vin.scriptSig.empty()) {
	    LogPrintf("masternode", "dsee - Ignore Not Empty ScriptSig %s\n", vin.prevout.hash.ToString());
            Misbehaving(pfrom->GetId(), 100);
            return;
        }
        ...
        if (!obfuScationSigner.VerifyMessage(pubkey, vchSig, strMessage, errorMessage)) {
            LogPrintf("masternode", "dsee - Got bad Masternode address signature\n");
            Misbehaving(pfrom->GetId(), 100);
            return;
        }
        ...
        if (!obfuScationSigner.IsVinAssociatedWithPubkey(vin, pubkey)) {
            LogPrintf("masternode", "dsee - Got mismatched pubkey and vin\n");
            Misbehaving(pfrom->GetId(), 100);
            return;
        }
	...
	LogPrint("masternode", "dsee - Got NEW OLD Masternode entry %s\n", vin.prevout.hash.ToString());
        ...
        if (fAcceptable) {
            if (GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS) {
                LogPrintf("masternode", "dsee - Input must have least %d confirmations\n", MASTERNODE_MIN_CONFIRMATIONS);
	    ...
	    LogPrintf("masternode", "dsee - Rejected Masternode entry %s\n", vin.prevout.hash.ToString());

            int nDoS = 0;
            if (state.IsInvalid(nDoS)) {
		    LogPrintf("masternode", "dsee - %s from %i %s was not accepted into the memory pool\n", tx.GetHash().ToString().c_str(),
        ...
        if (sigTime > GetAdjustedTime() + 60 * 60) {
	    LogPrintf("masternode", "dseep - Signature rejected, too far into the future %s\n", vin.prevout.hash.ToString());
	    Misbehaving(pfrom->GetId(), 1);
            return;
        }

        if (sigTime <= GetAdjustedTime() - 60 * 60) {
	    LogPrintf("masternode", "dseep - Signature rejected, too far into the past %s - %d %d \n", vin.prevout.hash.ToString(), sigTime, GetAdjustedTime());
            Misbehaving(pfrom->GetId(), 1);
            return;
        }
        ...
        if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
		    ...
		    LogPrintf("masternode", "dseep - Got bad Masternode address signature %s \n", vin.prevout.hash.ToString());
                    //Misbehaving(pfrom->GetId(), 100);
                    return;
                }
...
void CMasternodeMan::UpdateMasternodeList(CMasternodeBroadcast mnb)
{
    ...
    LogPrintf("masternode", "CMasternodeMan::UpdateMasternodeList -- masternode=%s  addr=%s\n", mnb.vin.prevout.ToStringShort(), mnb.addr.ToString());
