...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : BWKz spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : BWKz spend with serial %s is already in block %d\n",
		...
		/* Not Applicable: Bulwark
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : BWKz spend with serial %s from tx %s is not in valid range\n",
		...
		*/
            ...
            for (const CTxIn txin : tx.vin) {
              ...
              /** Check for invalid/fraudulent inputs (Not Applicable: Bulwark)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        ...
        } else if (nHeight <= 960 && nHeight > 0) {
            nSubsidy = 1000 * COIN;
        } else if (nHeight <= 7680 && nHeight > 960) {
            nSubsidy = 500 * COIN;
        } else if (nHeight <= 14400 && nHeight > 7680) {
            nSubsidy = 250 * COIN;
        } else if (nHeight <= 21120 && nHeight > 14400) {
            nSubsidy = 125 * COIN;
        } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 21120) {
            nSubsidy = 62.5 * COIN;
        } else if (nHeight <= 28800 && nHeight > Params().LAST_POW_BLOCK()) {
            nSubsidy = 50 * COIN;
        } else {
            nSubsidy = 0 * COIN;
        }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight <= 960 && nHeight > 0) {
            ret = 0;
        } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 960) {
            ret = blockValue / 4;
        }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Bulwark 
...
void RecalculateBWKZSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from BWKz supply
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
		...
		*/
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : BWKz with serial %s is already in the block %d\n",
            ...
            // Check that the inputs are not marked as invalid/fraudulent (Not Applicable: Bulwark)
            // for (CTxIn in : tx.vin) {
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s BWKzSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    //Continue tracking possible movement of fraudulent funds until they are completely frozen (Not Applicable) 
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BWK to BWKz
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    // Version 4 header must be used when SPORK_17_ENABLE_ZEROCOIN And never before.
    if (block.GetBlockTime() > GetSporkValue(SPORK_17_ENABLE_ZEROCOIN)) {
        ...
        if (block.nVersion >= Params().Zerocoin_HeaderVersion())
            return state.DoS(50, error("CheckBlockHeader() : block version must be below 4 before SPORK_17_ENABLE_ZEROCOIN"),
            REJECT_INVALID, "block-version");
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent BWKz spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of BWKz serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d BWKz mints and %d BWKz spends\n", __func__, nMints, nSpends);
