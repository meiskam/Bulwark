...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 BWK at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for BWKz send
enum ZerocoinSpendStatus {
    BWKZ_SPEND_OKAY = 0,                            // No error
    BWKZ_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    BWKZ_WALLET_LOCKED = 2,                         // Wallet was locked
    BWKZ_COMMIT_FAILED = 3,                         // Commit failed, reset status
    BWKZ_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    BWKZ_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    BWKZ_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    BWKZ_TRX_CREATE = 7,                            // Everything related to create the transaction
    BWKZ_TRX_CHANGE = 8,                            // Everything related to transaction change
    BWKZ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    BWKZ_INVALID_COIN = 10,                         // Selected mint coin is not valid
    BWKZ_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    BWKZ_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    BWKZ_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    BWKZ_SPENT_USED_BWKZ = 14,                      // Coin has already been spend
    BWKZ_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setBWKZAutoBackups(bool fEnabled)
    {
